...
// BASIC
template <typename T>
class ProtobufSerdes<T, typename std::enable_if<is_basic<T>::value && !is_extension_basic<T>::value>::type> : public Serdes {
...
// BASIC-envar
template <typename T>
class ProtobufSerdes<T, typename std::enable_if<std::is_same<T, envar>::value>::type> : public Serdes {
    virtual void serialize(std::shared_ptr<const Parameter> p, void* out) const override
    {
        auto parameter = std::static_pointer_cast<const TraitedParameter<T>>(p);
        auto& ctx = (*static_cast<detail::ProtobufSerdesContext*>(out));
        ...
        if (parent->is_repeated()) { // repeated 修饰的基本类型
            detail::protobuf_add_repeated_value(parent->message(), field, parameter->value.original());
        } else {
            detail::protobuf_set_value(parent->message(), field, parameter->value.original());
        }
    }
